...
if [ -z "${HOME:-}" ]; then
  ...
  [ -n "$HOME" ] || { printf 'K-pop Codex Theme: could not resolve the current macOS home directory.\n' >&2; exit 1; }
  export HOME
fi
...
INSTALL_ROOT="$HOME/.codex/codex-kpop-theme"
STATE_ROOT="$HOME/Library/Application Support/CodexKpopTheme"
...
CODEX_APP_JOB_LABEL="com.openai.codex-kpop-theme.app"
INJECTOR_JOB_LABEL="com.openai.codex-kpop-theme.injector"
