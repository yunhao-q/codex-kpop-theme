...
record_start_error() {
  ...
  printf 'K-pop Codex Theme: start failed at line %s (exit %s). See %s\n' "$line" "$code" "$START_ERROR_LOG" >&2
}
...
if [ "$verify_code" -ne 0 ]; then
  ...
  if /usr/bin/grep -q '"installed": true' /tmp/dream-skin-verify.$$.json 2>/dev/null; then
    printf 'K-pop Codex Theme %s is active (soft verify) on port %s.\n' "$SKIN_VERSION" "$PORT"
...
printf 'K-pop Codex Theme %s is active on loopback port %s.\n' "$SKIN_VERSION" "$PORT"
